...
export const CustomVideoTileGrid: React.FC<Props> = ({
  ...
  }, [changeState, meetingManager]);

  useEffect(() => {
    console.log('war asasasbai', tiles.length);
    addClassForVideo(tiles.length);
  }, [tiles.length]);

  const changeState = (data: any) => {
    c = [];
    const listAttendee = data
      .replace('[', '')
      .replace(']', '')
      .split(',');
  ...
  const draw = () => {
    const init_x = 0;
    const init_y = 0;
    let big_img_width = 900;
    let big_img_height = 400;
    const small_img_width = 169;
    const small_img_height = 99;
    const gap = 0;
    ...
    // video.push(x1);
    const x2 = document.querySelectorAll('.VideoSectionEmpty Video');
      ...
      if (video.length < 4 || video.length === 5) {
        for (let i = 0; i < video.length; i++) {
          if (i === 0)
            ...
            ctx.drawImage(
              video[i],
              init_x + big_img_width + big_img_width,
              init_y,
              big_img_width,
              big_img_height
            );
            ...
            ctx.drawImage(
              video[i],
              init_x + big_img_width + big_img_width + big_img_width,
              init_y,
              big_img_width,
              big_img_height
            );
        ...
        for (let i = 0; i < video.length; i++) {
          if (i === 0)
            ...
            ctx.drawImage(
              video[i],
              init_x + big_img_width + big_img_width,
              init_y + big_img_height + big_img_height,
              big_img_width,
              big_img_height
            );
          } else if (i === 5) {
            ctx.drawImage(
              video[i],
              init_x + big_img_width + big_img_width + big_img_width,
              init_y + big_img_height + big_img_height + big_img_height,
              big_img_width,
              big_img_height
            );
          }
    ...
    <Fragment>
      ...
      <div className="DashboardMainContent">
        <div
          id="main_vdo_sec"
          className={
            attendeeIdList.length === 0 ? 'VideoSection' : 'VideoSectionEmpty'
          }
        >
          <div className={dynamicVideoClass}>
            ...
            {attendeeIdList
              .filter(o => o !== attendeeIdToTileId[featureTileId])
              .map(remoteAttendeeId => {
                const tileId = attendeeIdToTileId[remoteAttendeeId];
        ...
        <div className="videoMembersBottom">
          <div>
            <LocalVideo nameplate="Me" className="img-fluid_90" />
            {isHost ? (
              <div
              ...
              >
                <img style={{ width: '18px' }} src={offVideoIcon} />
              </div>
            ) : null}
          </div>
          {isHost
